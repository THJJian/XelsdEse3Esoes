(function($) {
    ...
    function getSelectedCells(target) {
        var cells = [];
        var panel = $(target).datagrid("getPanel");
        panel.find("td.datagrid-row-selected")
            .each(function() {
                var td = $(this);
                cells.push({
    ...
    function gotoCell(target, p) {
        ...
        if (p == "up" && rowIndex > 0) {
            ...
            if (opts.finder.getRow(target, rowIndex + 1)) {
                ...
                tableGrid.insertEmptyRow(++rowIndex);
                //opts.navHandler["40"].call(target, window.event);
            }
        } else if (p == "left") {
            var i = colIndex - 1;
            while (i >= 0) {
    ...
    function editCell(target, param) {
        ...
        if (col.editor) {
            ...
            if (input) {
                dg.datagrid("gotoCell", param);
                setTimeout(function() {
                        input.unbind(".cellediting")
                            .bind("keydown.cellediting",
                                function(e) {
                                    if (e.keyCode == 13) {
                                        opts.navHandler["13"].call(target, e);
                                        return false;
                                    }
                                    //TODO 8 沒有 return true;
                                    return true;
                                });
                        input.focus();
                    },
        ...
        panel.attr("tabindex", 1)
            ...
            .bind("keydown.cellediting",
                function(e) {
                    ...
                    //TODO 9 沒有 return null;
                    return null;
                });
        dc.body1.add(dc.body2)
            .unbind(".cellediting")
            .bind("click.cellediting",
                function(e) {
        ...
        panel.attr("tabindex", 1)
            ...
            .bind("keydown.cellediting",
                function(e) {
                    ...
                    //TODO 10 沒有 return null;
                    return null;
                })
            .bind("keypress.cellediting",
                function(e) {
    ...
    $.extend($.fn.datagrid.methods,
    {
        ...
        input: function (jq, param) {
            ...
            if (ed) {
                ...
                //var _editor_type = ed.type;
                //if (_editor_type === "datetimebox")
                //    t.prev().click();
                return t;
            } else {
                return null;
